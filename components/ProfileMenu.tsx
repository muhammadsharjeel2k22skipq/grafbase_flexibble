    ...
    <div className='flexCenter z-10 flex-col relative'>
      <Menu as='div'>
        <Menu.Button className='flexCenter' onMouseEnter={() => setOpenModal(true)}>
          {session?.user?.image && (
            <Image src={session?.user?.image} width={40} height={40} className='rounded-full' alt='login-image' />
          )}
        </Menu.Button>
        ...
        >
          <Menu.Items static className='flexStart profile_menu-items' onMouseLeave={() => setOpenModal(false)}>
            <div className='flex flex-col items-center gap-y-4'>
              {session?.user?.image && (
                <Image src={session?.user?.image} width={80} height={80} className='rounded-full' alt='profile-image' />
              )}
...
// {session?.user?.image && (
//     <Link href={`/profier/${session?.user?.id}`}>
