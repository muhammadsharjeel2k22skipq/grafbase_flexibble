  ...
  return (
    <section className='flex flex-col mt-32 w-full'>
      <div className='flexBetween'>
        <p className='text-base'>More by <span className='text-base font-bold'>{result?.user?.name}</span></p>
        ...
        {filteredProjects.map(({node}:{node:ProjectInterface}) => (
          <div key={node?.id} className='flexCenter related_project-card drop-shadow-card'>
            <Link href={`/project/${node?.id}`} className='flexCenter group relative w-full h-full'>
              <Image src={node?.image} width={414} height={314} alt='prodImg' className='w-full h-full object-cover rounded-2xl' />
